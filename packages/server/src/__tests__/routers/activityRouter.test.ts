...
describe('Activity Router', () => {
    ...
    return new Promise<void>((resolve) => {
      ...
      app.use(
        ...
        trpcExpress.createExpressMiddleware({
          router: appRouter,
          createContext: (opts: CreateExpressContextOptions): CustomContext => ({
            req: opts.req,
            res: opts.res,
            user: null,
          }),
  ...
  it('should get activities', async () => {
    ...
    const result = await client.activity.getActivities.query({ user_id: TEST_USER_ID });
  ...
  it('should get activity by id', async () => {
    ...
    expect(activityService.getActivityById).toHaveBeenCalledWith(TEST_ACTIVITY_ID);
  });
  ...
  it('should create a new activity with duration', async () => {
    ...
    const result = await client.activity.createActivity.mutate({
      user_id: TEST_USER_ID,
      activity: newActivity,
    });

    expect(result).toEqual(createdActivity);
    expect(activityService.createActivity).toHaveBeenCalledWith(TEST_USER_ID, {
      ...newActivity,
  ...
  it('should handle invalid input for createActivity', async () => {
    const invalidActivity: ExpectedActivityInput = {
      user_id: TEST_USER_ID,
      activity: {
        date: 'invalid-date',
        activity: '',
    ...
    await expect(client.activity.createActivity.mutate(invalidActivity as { user_id: number; activity: { date: string; activity: string; duration?: number | undefined; notes?: string | undefined; }; }))
    .rejects.toThrow();
});
  ...
  it('should handle database errors', async () => {
    ...
    await expect(client.activity.getActivities.query({ user_id: TEST_USER_ID }))
      .rejects.toThrow('Database error');
  });

  it('should handle activities with and without duration in getActivities', async () => {
    const userId = TEST_USER_ID;
    const mockActivities: ActivityTable[] = [
      { id: 1, user_id: userId, date: '2023-01-01', activity: 'Running', duration: 30, notes: 'Good run' },
      { id: 2, user_id: userId, date: '2023-01-02', activity: 'Meditation', notes: 'Peaceful' },
    ];

    vi.mocked(activityService.getActivities).mockResolvedValue(mockActivities);

    const result = await client.activity.getActivities.query({ user_id: userId });

    expect(result).toEqual(mockActivities);
    expect(activityService.getActivities).toHaveBeenCalledWith(userId);
  });

  it('should handle creating an activity with minimum required fields', async () => {
    const newActivity: ActivityInput = {
      date: '2023-01-01',
      activity: 'Reading',
      notes: 'Good book'
