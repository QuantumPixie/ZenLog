...
describe('Activity Service Integration Tests', () => {
  ...
  it('should create and retrieve an activity', async () => {
    ...
    const createdActivity = await activityService.createActivity(userId, newActivity);
    expect(createdActivity).toHaveProperty('id');
    expect(createdActivity.activity).toBe(newActivity.activity);

    const retrievedActivity = await activityService.getActivityById(createdActivity.id, userId);
  ...
  it('should get activities by date range', async () => {
    ...
    for (const activity of activities) {
      await activityService.createActivity(userId, activity);
    }

    const retrievedActivities = await activityService.getActivitiesByDateRange(userId, '2024-08-01', '2024-08-02');
