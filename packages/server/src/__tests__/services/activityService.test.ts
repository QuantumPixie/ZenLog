...
describe('activityService', () => {
  ...
  describe('createActivity', () => {
    it('should create and return a new activity with duration', async () => {
      const userId = 1;
      const newActivity = { date: '2023-07-27', activity: 'Running', duration: 30, notes: 'Good run' };
      ...
      expect(mockKysely.insertInto('activities').values).toHaveBeenCalledWith(expect.objectContaining({
        user_id: userId,
        ...newActivity,
      }));
    });
    ...
    it('should create and return a new activity without duration', async () => {
      const userId = 1;
      const newActivity = { date: '2023-07-27', activity: 'Meditation', notes: 'Peaceful' };
  ...
  describe('getActivitiesByDateRange', () => {
    it('should return activities within the specified date range', async () => {
      ...
      const mockActivities = [
        { id: 1, user_id: 1, date: '2023-07-27', activity: 'Running', duration: 30, notes: 'Good run' },
        { id: 2, user_id: 1, date: '2023-07-28', activity: 'Yoga', notes: 'Relaxing session' },
      ];
      ...
      expect(mockKysely.selectFrom).toHaveBeenCalledWith('activities');
      expect(mockKysely.selectFrom('activities').selectAll).toHaveBeenCalled();
      expect(mockKysely.selectFrom('activities').selectAll().where).toHaveBeenCalledTimes(3);
    });
  ...
  describe('error handling', () => {
    it('should throw an error if database query fails in getActivities', async () => {
      ...
      await expect(activityService.getActivities(userId)).rejects.toThrow('Database error');
    });
    ...
    it('should throw an error if activity creation fails', async () => {
      ...
      await expect(activityService.createActivity(userId, newActivity)).rejects.toThrow('Failed to create activity');
    });
  });
