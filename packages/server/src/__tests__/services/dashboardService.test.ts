...
describe('dashboardService', () => {
  ...
  describe('getSummary', () => {
    it('should return a summary for a user', async () => {
      const userId = 1;
      const mockMoods = [{ date: new Date(), mood_score: 7, emotions: ['happy'] }];
      const mockEntries = [{ date: new Date(), entry: 'Test entry' }];
      const mockActivities = [{ date: new Date(), activity: 'Running', duration: 30 }];
      ...
      expect(mockChain.select).toHaveBeenNthCalledWith(3, ['date', 'activity', 'duration']);
      ...
      expect(mockChain.where).toHaveBeenCalledWith('date', '>=', expect.any(Date));
