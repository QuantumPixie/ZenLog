...
describe('moodService', () => {
  ...
  describe('getMoods', () => {
    it('should return moods for a user', async () => {
      const userId = 1;
      const mockMoods = [
        { id: 1, date: '2023-08-03', moodScore: 7, emotions: ['happy', 'excited'] },
      ...
      expect(mockKysely.selectFrom('moods').select).toHaveBeenCalledWith(['id', 'date', 'moodScore', 'emotions']);
      expect(mockKysely.selectFrom('moods').select(['id', 'date', 'moodScore', 'emotions']).where).toHaveBeenCalledWith('user_id', '=', userId);
      expect(mockKysely.selectFrom('moods').select(['id', 'date', 'moodScore', 'emotions']).where('user_id', '=', userId).orderBy).toHaveBeenCalledWith('date', 'desc');
    });
  });
  ...
  describe('createMood', () => {
    it('should create a new mood entry', async () => {
      const userId = 1;
      const moodData = { date: '2023-08-03', moodScore: 8, emotions: ['happy', 'relaxed'] };
      ...
      expect(mockKysely.insertInto('moods').values(expect.any(Object)).returning).toHaveBeenCalledWith(['id', 'date', 'moodScore', 'emotions']);
    });
  });
  ...
  describe('getMoodsByDateRange', () => {
    it('should return moods within a date range', async () => {
      ...
      expect(mockKysely.selectFrom('moods').select).toHaveBeenCalledWith(['id', 'date', 'moodScore', 'emotions']);
      expect(mockKysely.selectFrom('moods').select(['id', 'date', 'moodScore', 'emotions']).where).toHaveBeenCalledWith('user_id', '=', userId);
      expect(mockKysely.selectFrom('moods').select(['id', 'date', 'moodScore', 'emotions']).where('user_id', '=', userId).where).toHaveBeenCalledWith('date', '>=', startDate);
      expect(mockKysely.selectFrom('moods').select(['id', 'date', 'moodScore', 'emotions']).where('user_id', '=', userId).where('date', '>=', startDate).where).toHaveBeenCalledWith('date', '<=', endDate);
      expect(mockKysely.selectFrom('moods').select(['id', 'date', 'moodScore', 'emotions']).where('user_id', '=', userId).where('date', '>=', startDate).where('date', '<=', endDate).orderBy).toHaveBeenCalledWith('date', 'asc');
    });
  });
});